...
<table class="table">
    ...
    <tbody>
    <?php 
        $table = mysqli_query($con, "SELECT * FROM supplier_cab WHERE supplier_id = '$supplier_id'");
    ...
    ?>
            ...
            <td data-target="price_km"><?php echo $row['price_km'] ?></td>
            <td data-target="total_price"><?php echo $row['total_price'] ?></td>
            <td><a href="#" data-role="update" data-id="<?php echo $row['id']; ?>"><button class="btn btn-primary"> Update</button></a></td>
            <td><a href="details/transfers.php?lid=<?php echo $row['id']; ?>"><button class="btn btn-primary"> Show Details</button></a></td>
        </tr>
        <?php
        }
        ?>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            ...
            <input type="text" id="price_km" value="<?php echo $row['price_km'] ?>" class="form-control"><br>
            <label>Total Price</label>
            <input type="text" id="total_price" value="<?php echo $row['total_price'] ?>" class="form-control">
...
$(document).ready(function(){
    $(document).on('click','a[data-role=update]',function(){
        var id = $(this).data('id');
        var price_km = $('#'+id).children('td[data-target=price_km]').text();
        var total_price = $('#'+id).children('td[data-target=total_price]').text();

        $('#price_km').val(price_km);
    ...
    $('#save').click(function(){
        ...
        $.ajax({
            url:        'connection.php',
            method:     'POST',
            data:       { price_km: price_km, total_price: total_price, id: id},
            success:    function(response){
                            $('#'+id).children('td[data-target=price_km]').text(price_km);
                            $('#'+id).children('td[data-target=total_price]').text(total_price);
                            $('#myModal3').modal('toggle');
                        }
        });
    });
